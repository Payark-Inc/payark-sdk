...
describe("HttpClient", () => {
  ...
  describe("error handling", () => {
    test("should throw PayArkError with authentication_error on 401", async () => {
      ...
      setFetch(
        mock(() =>
          Promise.resolve(
            mockResponse({ error: "Unauthorized: Invalid API Key" }, 401),
          ),
        ),
      );
    ...
    test("should throw PayArkError with permission_error on 403", async () => {
      const client = createClient();
      setFetch(
        mock(() => Promise.resolve(mockResponse({ error: "Forbidden" }, 403))),
      );
      ...
      try {
        ...
        expect((err as PayArkError).code).toBe("permission_error");
    ...
    test("should throw PayArkError with invalid_request_error on 400", async () => {
      ...
      try {
        ...
        expect((err as PayArkError).raw?.details).toEqual({
          amount: "Required",
        });
      }
    });
    ...
    test("should handle non-JSON error response body gracefully", async () => {
      const client = createClient();
      setFetch(
        mock(() => Promise.resolve(textResponse("Internal Server Error", 500))),
      );
    ...
    test("should throw connection_error on DNS/connection failure", async () => {
      const client = createClient();
    ...
    test("should throw connection_error on TypeError (fetch not available)", async () => {
      const client = createClient();
      setFetch(
        mock(() => Promise.reject(new TypeError("fetch is not a function"))),
      );
  ...
  describe("retry behaviour", () => {
    ...
    test("should NOT retry on 404", async () => {
      ...
      setFetch(
        mock(() => Promise.resolve(mockResponse({ error: "Not Found" }, 404))),
      );
      ...
      setFetch(
        mock(() => {
          ...
          return Promise.resolve(
            mockResponse({ error: "Slow down" }, 429, { "Retry-After": "1" }),
          );
        }),
      );
    ...
    test("should use maxRetries=0 to disable retrying", async () => {
      ...
      setFetch(
        mock(() => Promise.resolve(mockResponse({ error: "Error" }, 500))),
      );
